...
class InverseKinematics(Node):
    ...
    def __init__(self):
        super().__init__('inverse_kinematics_A')
    ...
    def forward_kinematics(self, q):
        q1, q2, q3 = q
        x = 1.5 * (np.cos(q2) + np.cos(q2 + q3)) * np.cos(q1)
        y = 1.5 * (np.cos(q2) + np.cos(q2 + q3)) * np.sin(q1)
        z = 1.5 * np.sin(q2) + 1.5 * np.sin(q2 + q3) + 3.0
        return np.array([x, y, z])

    def jacobian(self, q):
        q1, q2, q3 = q

        j11 = -1.5 * (np.cos(q2) + np.cos(q2 + q3)) * np.sin(q1)
        j12 = -1.5 * (np.sin(q2) + np.sin(q2 + q3)) * np.cos(q1)
        j13 = -1.5 * np.sin(q2 + q3) * np.cos(q1)

        j21 =  1.5 * (np.cos(q2) + np.cos(q2 + q3)) * np.cos(q1)
        j22 = -1.5 * (np.sin(q2) + np.sin(q2 + q3)) * np.sin(q1)
        j23 = -1.5 * np.sin(q2 + q3) * np.sin(q1)

        j31 =  0
        j32 =  1.5 * np.cos(q2) + 1.5 * np.cos(q2 + q3)
        j33 =  1.5 * np.cos(q2 + q3)
